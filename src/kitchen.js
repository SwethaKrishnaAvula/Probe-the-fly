...
import { sceneFor } from './kitchenScenes.js';
...
export function createKitchen(scene) {
  ...
  let sc = sceneFor('level_1_discovery'); // the current level's arrangement
  let sinkHole = null;
  ...
  const world = {
    ...
    colliders,
    sinkRect: null,
    start: toWorld(sceneFor('level_1_discovery').start),
  ...
  return {
    ...
    configure(cfg, levelId, variant = 0) {
      sc = sceneFor(levelId, variant);
      F = sc.mirrored ? -1 : 1;
      buildLevel(levelId);
